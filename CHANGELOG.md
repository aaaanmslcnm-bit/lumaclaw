...
## LumaClaw public release line updates (unreleased)

### 2026-03-08

Current LumaClaw/SoulClaw repository work includes:

- added SoulBond MVP documentation, demo documentation, public-facing messaging pack, facade copy lock-in, and a release-prep checklist under the planning docs
- added a repository-level `ROADMAP.md`
- rewrote the top-level `README.md`, `CONTRIBUTING.md`, and `VISION.md` into SoulClaw project language
- updated `SECURITY.md`, `docs.acp.md`, Fly templates, Render template, Docker templates, and related release-facing files with SoulClaw fork-stage wording
- changed `package.json` public package identity to `soulclaw`
- adopted a dual-CLI transition strategy: `soulclaw` + legacy `openclaw`
- clarified that SoulClaw currently keeps inherited upstream runtime/docs surfaces while the fork boundary is being cleaned up deliberately

These notes are the beginning of a SoulClaw-specific changelog layer.
They sit above the inherited upstream OpenClaw history below.

## SoulClaw fork-stage note

SoulClaw is currently built directly on top of OpenClaw.

This changelog still contains a large amount of inherited upstream OpenClaw release history.
That does **not** mean every entry below is a SoulClaw-specific change.

For the current stage, read this file as:

- historical upstream base reference
- inherited runtime/platform context
- not yet a clean SoulClaw-only release ledger

As the fork boundary becomes cleaner, SoulClaw-specific release history should be recorded separately and more explicitly.

Docs: inherited upstream references may still point to OpenClaw while the fork is being separated.
