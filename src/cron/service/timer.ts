...
import { formatSoulBondRunAsSystemEvent } from "../../soulbond/cron-adapter.js";
import { parseSoulBondDailySystemEventText } from "../../soulbond/cron-job.js";
import { runSoulBondDaily } from "../../soulbond/run-daily.js";
...
> {
  ...
  if (job.sessionTarget === "main") {
    ...
    const soulBondStatePath = parseSoulBondDailySystemEventText(text);
    if (soulBondStatePath) {
      const soulBondResult = await runSoulBondDaily({
        statePath: soulBondStatePath,
        now: new Date(state.deps.nowMs()),
      });
      text = formatSoulBondRunAsSystemEvent(soulBondResult);
    }
